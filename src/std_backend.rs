...
pub(crate) fn install(blueprint_root: &Path) -> Result<(), String> {
    ...
    let status = Command::new("python3").arg(&contract_check)
        .arg("--kernel").arg(&kernel).arg("--blueprints").arg(blueprint_root)
        .status().map_err(|error| format!("cannot run {}: {error}", contract_check.display()))?;
    if !status.success() { return Err("native worker source ABI verification failed".into()); }
    let installer = kernel.join("tools/apply_trueos_rust_std_thread_backend.py");
    let output = Command::new("python3").arg(&installer).arg(&source_root).output()
        .map_err(|error| format!("cannot run {}: {error}", installer.display()))?;
    if !output.status.success() {
        return Err(format!("TRUEOS std backend installation failed: {}{}",
...
pub(crate) fn cache_revision() -> Result<String, String> {
    ...
    for relative in [
        "std/src/sys/thread/mod.rs", "std/src/sys/thread/trueos.rs",
        "std/src/os/unix/mod.rs", "std/src/thread/current.rs",
        "std/src/sys/thread_local/mod.rs", "std/src/sys/thread_local/no_threads.rs",
        "std/src/hash/random.rs", "std/src/sys/pal/unix/time.rs",
    ] {
