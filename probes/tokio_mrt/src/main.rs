...
enum Event { Ready(usize, u32), Step(usize, u64) }

fn main() {
    let runtime = match t::runtime::current_thread().build() {
        Ok(runtime) => runtime,
        Err(error) => { logl::log(level::ERROR, format_args!("tokio_mrt: FAIL runtime={error}")); return; }
    };
    let result = runtime.block_on(async {
        for wave in 0..WAVES { run_wave(wave).await?; }
        Ok::<_, &'static str>(())
    });
    match result {
        Ok(()) => logl::log(level::INFO, format_args!("tokio_mrt: PASS lanes={LANES} waves={WAVES} tasks={TASKS} rounds={ROUNDS}")),
        Err(stage) => logl::log(level::ERROR, format_args!("tokio_mrt: FAIL stage={stage}")),
    }
}

async fn run_wave(wave: usize) -> Result<(), &'static str> {
    if t::worker::capacity() < LANES { return Err("insufficient-native-capacity"); }
    ...
    for lane in 0..LANES {
        ...
        match t::worker::spawn(move || {
            ...
            let result = runtime.block_on(async {
                ...
                tx.send(Event::Ready(lane, slot)).await.map_err(|_| "ready.send")?;
                t::time::timeout(DEADLINE, async {
                    while !release.load(Ordering::Acquire) { t::time::sleep(Duration::from_millis(1)).await; }
                }).await.map_err(|_| "release.timeout")?;
                if cancel.load(Ordering::Acquire) { return Err("cancelled-before-start"); }
                ...
                for task in 0..TASKS {
                    ...
                    tasks.spawn(async move {
                        for round in 0..ROUNDS {
                            ...
                            if t::worker::local_slot() != slot { return Err("unstable-worker-slot"); }
                            let value = (((wave * LANES + lane) * TASKS + task) * ROUNDS + round + 1) as u64;
                            tx.send(Event::Step(lane, value)).await.map_err(|_| "step.send")?;
                        }
                        Ok::<_, &'static str>(())
                    });
                }
                while let Some(result) = tasks.join_next().await { result.map_err(|_| "task.join")??; }
                Ok(slot)
            });
            drop(runtime);
            result
    ...
    if error.is_none() {
        for _ in 0..LANES {
            match t::time::timeout(DEADLINE, rx.recv()).await {
                Ok(Some(Event::Ready(lane, slot))) if lane < LANES && slots[lane].is_none() => slots[lane] = Some(slot),
                _ => { error = Some("ready.timeout-or-protocol"); break; }
            }
        }
    }
    if error.is_some() { cancel.store(true, Ordering::Release); rx.close(); }
    ...
    let receive_result = t::time::timeout(DEADLINE, async {
        while let Some(event) = rx.recv().await {
            match event {
                Event::Step(lane, value) if lane < LANES => { counts[lane] += 1; checksum += value; }
                _ => { error.get_or_insert("event.protocol"); }
            }
        }
    }).await;
    ...
    for (lane, mut job) in jobs.into_iter().enumerate() {
        let joined = match t::time::timeout(DEADLINE, &mut job).await {
            Ok(result) => result,
            Err(_) => { error.get_or_insert("join.timeout"); cancel.store(true, Ordering::Release); job.await }
        };
        match joined {
            Ok(Ok(slot)) if slots[lane] == Some(slot) => {}
            Ok(Err(stage)) => { error.get_or_insert(stage); }
            _ => { error.get_or_insert("join.result"); }
        }
    }
    ...
    logl::log(level::INFO, format_args!("tokio_mrt: wave={wave} counts={counts:?} checksum={checksum} elapsed_ms={} slots={slots:?}", started.elapsed().as_millis()));
    error.map_or(Ok(()), Err)
}
