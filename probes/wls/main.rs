...
fn main() {
    ...
    match result {
        Ok(()) => logl::log(
            level::INFO,
            format_args!("wls: PASS native.isolation runtime.reuse std.spawn.unsupported"),
        ),
...
async fn run_probe() -> Result<(), &'static str> {
    ...
    for index in 0..2u64 {
        ...
        let job = t::worker::spawn(move || {
            ...
            let previous = MARKER.with(|marker| {
    ...
    if error.is_none()
        ...
        .is_err()
    {
        error = Some("coordinator.ready.timeout");
    }
        ...
        match joined {
            Ok(Ok((slot, previous, mark))) => {
                logl::log(
                    level::INFO,
                    format_args!(
                        "wls: slot={slot} previous={previous:x} mark={mark:x} runtime_cycles=2"
                    ),
                );
                slots.push(slot);
            }
            ...
            Err(_) => {
                error.get_or_insert("worker.join");
            }
        }
    }
    ...
    if MARKER.with(Cell::get) != MAIN_MARK {
        error.get_or_insert("coordinator.isolation");
    }
    error.map_or(Ok(()), Err)
}
