...
impl Frame {
    ...
    pub fn open_layered(
        ...
        let window_id = unsafe {
            v::bp_abi::trueos_cabi_ui4_scene_frame_open_layered_v1(
                x,
                y,
                width,
                height,
    ...
    pub fn background(&self) -> Result<BackgroundLayer, Error> {
        ...
        Ok(BackgroundLayer {
            surface: core::mem::ManuallyDrop::new(Self {
                window_id: target,
                width: self.width,
                height: self.height,
            }),
    ...
    pub fn set_primary_activation(&mut self, enabled: bool) -> Result<(), Error> {
        status(unsafe {
            v::bp_abi::trueos_cabi_ui4_scene_frame_primary_activation(
                self.window_id,
                enabled as u32,
            )
        })
    ...
    pub fn position(&self) -> Result<(i32, i32), Error> {
        let mut xy = [0i32; 2];
        status(unsafe {
            v::bp_abi::trueos_cabi_ui4_scene_frame_get_position(self.window_id, xy.as_mut_ptr())
        })?;
        Ok((xy[0], xy[1]))
    }
...
impl BackgroundLayer {
    ...
    pub fn begin_visual_gpu_frame(&mut self) -> Result<(), Error> {
        self.surface.begin_visual_gpu_frame()
    }
    pub fn register_shadertoy(&mut self, shader_id: u32, package: &[u8]) -> Result<(), Error> {
    ...
    pub fn publish_compute(&mut self, damage: Damage) -> Result<(), Error> {
        self.surface.publish_compute(damage)
    }
    pub fn publish(&mut self, damage: Damage) -> Result<(), Error> {
