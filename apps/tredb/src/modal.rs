...
pub enum ConfirmPurpose {
    ...
    DeleteRow { table: String, key: Vec<u8> },
    Reset { demo: bool },
}
...
pub fn byte_index(text: &str, character_index: usize) -> usize {
    text.char_indices()
        .nth(character_index)
        .map(|(index, _)| index)
        .unwrap_or(text.len())
...
mod tests {
    ...
    #[test]
    fn maps_character_cursor_to_bytes() {
        assert_eq!(byte_index("aéz", 0), 0);
        assert_eq!(byte_index("aéz", 1), 1);
        assert_eq!(byte_index("aéz", 2), 3);
        assert_eq!(byte_index("aéz", 3), 4);
    }
}
