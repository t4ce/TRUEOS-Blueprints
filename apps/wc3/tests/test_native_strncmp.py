#!/usr/bin/env python3
"""Run emitted guest strncmp machine code on Linux IA32, including ABI checks."""
...
with tempfile.TemporaryDirectory(prefix='wc3-strncmp-') as directory:
    work = Path(directory)
...
fn main() {{
    ...
    thunk32::write(7, thunk32::Kind::Strncmp, &mut thunk).unwrap();
...
'''
    ...
    print(f'PASS: {len(rows) * 2} native strncmp cases; unsigned bytes, NUL/count/page boundaries, cdecl, preserved registers and DF')
