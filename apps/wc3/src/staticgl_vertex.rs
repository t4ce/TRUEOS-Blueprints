...
fn gl_normal_matrix(matrix: &[f32; 16]) -> Result<[[f32; 3]; 3], &'static str> {
    ...
    if !determinant.is_finite() || determinant.abs() < 1e-20 {
        return Err("singular normal transform");
    }
...
fn gl_lit_color(
    ...
    let fixed = &c.fixed;
    if !fixed.is_enabled(0x0b50) {
        return Ok(color.map(|v| v.clamp(0.0, 1.0)));
    }
    if fixed.light_model_two_side {
        return Err("two-sided lighting needs back-face vertex colors");
    }
    ...
    if eye[3] == 0.0 {
        return Err("lighting vertex at infinity");
    }
    let eye_position = [eye[0] / eye[3], eye[1] / eye[3], eye[2] / eye[3]];
    let mut result = core::array::from_fn::<_, 4, _>(|i| {
        material.emission[i] + material.ambient[i] * c.light_model_ambient[i]
    });
    let viewer = if fixed.light_model_local_viewer {
        gl_vec_normalize(eye_position.map(|v| -v))
    } else {
        [0.0, 0.0, 1.0]
    };
    for (index, light) in fixed.lights.iter().enumerate() {
        if !fixed.is_enabled(0x4000 + index as u32) {
            continue;
        }
        let p = light.position_eye;
        let delta = if p[3] == 0.0 {
            [p[0], p[1], p[2]]
        } else {
        ...
        let attenuation = if p[3] == 0.0 {
            ...
            if denominator > 0.0 {
                ...
                return Err("zero light attenuation denominator");
            }
        };
        let spot = if light.spot_cutoff == 180.0 {
            1.0
        } else {
            let d = gl_vec_dot(
                direction.map(|v| -v),
                gl_vec_normalize(light.spot_direction_eye),
            )
            .max(0.0);
            if d < light.spot_cutoff.to_radians().cos() {
                0.0
            } else {
                d.powf(light.spot_exponent)
            }
        };
        ...
        for i in 0..3 {
            result[i] += attenuation
                * spot
                * (material.ambient[i] * light.ambient[i]
    ...
    if !result.iter().all(|v| v.is_finite()) {
        return Err("nonfinite lighting result");
    }
...
mod staticgl_vertex_tests {
    ...
    fn inverse_transpose_normal_transform_handles_nonuniform_scale() {
        let mut m = GL_IDENTITY_MATRIX;
        m[0] = 2.0;
        m[5] = 4.0;
        m[10] = 8.0;
        assert_eq!(
            gl_transform_normal(&gl_normal_matrix(&m).unwrap(), [1.0; 3]),
            [0.5, 0.25, 0.125]
        );
        m[5] = 0.0;
        assert!(gl_normal_matrix(&m).is_err());
    }
}
