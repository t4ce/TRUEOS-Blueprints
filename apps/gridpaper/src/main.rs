...
fn main() {
    let (grid_size, scale_percent) = match requested_grid_request() {
        Ok(request) => request,
        Err(error) => {
    ...
    *next_print_status_ms = clock::monotonic_millis().saturating_add(PRINT_STATUS_INTERVAL_MS);
    ...
    match resume {
        Ok(resume) => logl::log(
            level::INFO,
            format_args!(
                "gridpaper: Resume instance={} lineage={} generation={} clone={}; page resubmitted",
...
fn requested_grid_request() -> Result<(trueos::gridpaper::GridSize, u16), &'static str> {
    let mut args = env::args().skip(1);
    let Some(first) = args.next() else {
        return Ok((trueos::gridpaper::GridSize::FULL, 100));
    };
    ...
    let scale_percent = match args.next() {
        Some(value) => value
            .strip_suffix('%')
            .unwrap_or(value.as_str())
            .parse::<u16>()
            .ok()
            .filter(|scale| (1..=800).contains(scale))
            .ok_or("gridpaper: scale must be within 1% and 800%")?,
        None => 100,
    };
    if args.next().is_some() {
        return Err("gridpaper: expected COLUMNSxROWS followed by optional SCALE%");
    }
    let columns = columns
        .parse::<usize>()
    ...
    let size = trueos::gridpaper::GridSize::new(columns, rows)
        .map_err(|_| "gridpaper: grid size must be within 1x1 and 39x55")?;
    Ok((size, scale_percent))
}

fn submit_to_kernel(page: &GridPaper, grid_size: trueos::gridpaper::GridSize) {
