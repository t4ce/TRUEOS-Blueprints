...
pub extern "C" fn main() {
    let Some(window) = open_window() else {
        logl::log(level::ERROR, "chart bp: window create failed\n");
        return;
    };
    ...
    if !gfx::upload_texture_rgba_image_now(TEX_ID, WINDOW_WIDTH, WINDOW_HEIGHT, pixels.as_slice()) {
        logl::log(level::ERROR, "chart bp: texture upload failed\n");
        return;
    }
    let _ = window.id().request_repaint();
    logl::log(level::INFO, "chart bp: rendered sine axis chart\n");
