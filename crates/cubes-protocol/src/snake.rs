//! Five stable c1 cube slots. Each step replaces only the tail slot with a head.
...
impl State {
    pub fn valid(self) -> bool {
        (self.head as usize) < SEGMENTS
            ...
            && (0..SEGMENTS).all(|i| !self.cells[i + 1..].contains(&self.cells[i]))
            && (1..SEGMENTS).all(|i| {
                adjacent(
...
impl Replica {
    pub fn snapshot(&mut self, state: State) -> bool {
        if !state.valid() {
            return false;
        }
