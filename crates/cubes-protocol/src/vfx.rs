...
pub const INTERVAL_MS: u16 = 3000;
...
impl<'a> Sequence<'a> {
    pub fn parse(bytes: &'a [u8]) -> Option<Self> {
        ...
        if sequence.period_ms() == 0 || sequence.period_ms() as u32 * sequence.frame_count() as u32
            + DELAY_MS as u32 >= INTERVAL_MS as u32 { return None; }
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct Scene {
    pub gallery_revision: u32, pub event: u32, pub age_ms: u16,
...
mod tests {
    ...
    fn scene_roundtrip_order_and_exact_timing_boundaries() {
        ...
        next.age_ms=3000;assert!(Scene::parse(&next.encode()).is_none());
