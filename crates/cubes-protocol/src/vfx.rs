...
pub struct Pixel { pub x: u8, pub y: u8, pub palette: u8 }

/// A stable pixel identity across all frames in its compressed lifetime.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct Lifetime { pub pixel: Pixel, pub first: u8, pub end: u8 }

#[derive(Clone, Copy)]
pub struct Sequence<'a> { bytes: &'a [u8], data: usize }
impl<'a> Sequence<'a> {
    pub fn parse(bytes: &'a [u8]) -> Option<Self> {
        if bytes.len() < HEADER || bytes.len() > MAX_BYTES || &bytes[..4] != b"VFX1"
            || bytes[4..7] != [1, WIDTH, HEIGHT] || bytes[7] == 0 || bytes[10] == 0 || bytes[11] != 0
        { return None; }
        let data = HEADER + bytes[10] as usize * 3;
        if data > bytes.len() || (bytes.len()-data)%5 != 0 { return None; }
        let sequence = Self { bytes, data };
        if sequence.period_ms() == 0 || sequence.period_ms() > PERIOD_MS { return None; }
        let mut ends = [0;CELLS];
        for r in bytes[data..].chunks_exact(5) {
            if r[0]>=WIDTH || r[1]>=HEIGHT || r[2]>=bytes[10]
                || r[3]>=r[4] || r[4]>bytes[7] { return None; }
            let cell=r[1] as usize*32+r[0] as usize;
            if r[3]<ends[cell] { return None; }
            ends[cell]=r[4];
        }
        Some(sequence)
    }
    pub fn frame_count(self) -> u8 { self.bytes[7] }
    pub fn period_ms(self) -> u16 { u16::from_le_bytes([self.bytes[8],self.bytes[9]]) }
    pub fn lifetimes(self) -> impl Iterator<Item=Lifetime> + 'a {
        self.bytes[self.data..].chunks_exact(5).map(|r| Lifetime {
            pixel:Pixel{x:r[0],y:r[1],palette:r[2]},first:r[3],end:r[4]})
    }
    pub fn pixels(self, frame: u8) -> impl Iterator<Item=Pixel> + 'a {
        self.bytes[self.data..].chunks_exact(5)
            .filter(move |r| r[3]<=frame && frame<r[4])
            .map(|r| Pixel{x:r[0],y:r[1],palette:r[2]})
    }
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct Scene {
    pub gallery_revision: u32, pub event: u32, pub age_ms: u32,
...
impl Scene {
    ...
    pub fn parse(b: &[u8]) -> Option<Self> {
        ...
        let scene=Self {gallery_revision:u32::from_le_bytes(b[..4].try_into().unwrap()),
            ...
            age_ms:u32::from_le_bytes(b[8..12].try_into().unwrap()),slots};
        if scene.age_ms>=scene.batch_ms() || slots.iter().any(|s| s.bytes<HEADER as u32 || s.bytes>MAX_BYTES as u32
            || !PIXEL_SIDES_C1.contains(&s.pixel_side_c1) || s.frames==0 || s.period_ms==0 || s.period_ms>PERIOD_MS
...
mod tests {
    ...
    fn longest_loop_controls_next_batch_with_exact_one_second_vfx_gap() {
        ...
        let bytes=[b'V',b'F',b'X',b'1',1,32,32,255,144,1,1,0,255,255,255];
    ...
    fn lifetimes_retain_change_disappear_and_reappear() {
        let bytes = [
            b'V',b'F',b'X',b'1',1,32,32,5,150,0,2,0,255,0,0,0,255,0,
            2,3,0,0,3, 2,3,1,3,4, 4,5,0,0,1, 4,5,0,4,5,
        ];
        let seq=Sequence::parse(&bytes).unwrap();
        assert_eq!(seq.lifetimes().next().unwrap(),Lifetime {pixel:Pixel{x:2,y:3,palette:0},first:0,end:3});
        assert_eq!(seq.pixels(0).count(),2);
        for frame in [1,2] { assert_eq!(seq.pixels(frame).collect::<alloc::vec::Vec<_>>(),[Pixel{x:2,y:3,palette:0}]); }
        assert_eq!(seq.pixels(3).next().unwrap().palette,1);
        assert_eq!(seq.pixels(4).next().unwrap(),Pixel{x:4,y:5,palette:0});
        assert_eq!(seq.pixels(5).count(),0);
        for (index,value) in [(5,64),(8,0),(10,0),(18,32),(20,2),(21,3),(22,6),(26,2)] {
            let mut bad=bytes;bad[index]=value;assert!(Sequence::parse(&bad).is_none(),"{index}");
        }
    ...
    fn scene_roundtrip_order_and_exact_timing_boundaries() {
        ...
        assert_eq!(Scene::parse(&scene.encode()),Some(scene));
        let mut next=scene;next.event=0;
        assert!(next.newer_than(scene));assert!(!scene.newer_than(next));
        next=scene;next.age_ms=50;assert!(next.newer_than(scene));
        assert!(!scene.newer_than(scene));
        next.age_ms=scene.batch_ms();assert!(Scene::parse(&next.encode()).is_none());
        next=scene;next.slots[1].period_ms=0;
