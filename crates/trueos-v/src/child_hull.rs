...
pub fn spawn(initial: &[u8]) -> Result<Handle, i32> {
    let mut handle = 0;
    let rc = unsafe {
        bp_abi::trueos_cabi_blueprint_child_spawn_v1(
            initial.as_ptr(), initial.len(), &mut handle,
        )
    };
    if rc == 0 && handle != HANDLE_PARENT { Ok(handle) } else { Err(if rc == 0 { ERR_UNAVAILABLE } else { rc }) }
}

pub fn send(handle: Handle, bytes: &[u8]) -> Result<usize, i32> {
    let result = unsafe {
        bp_abi::trueos_cabi_blueprint_child_send_v1(handle, bytes.as_ptr(), bytes.len())
    };
...
pub fn receive_len(handle: Handle) -> Result<Option<usize>, i32> {
    let result = unsafe {
        bp_abi::trueos_cabi_blueprint_child_receive_v1(handle, core::ptr::null_mut(), 0)
    };
    if result < 0 { Err(result as i32) }
    else if result == 0 { Ok(None) }
    else { Ok(Some(result as usize)) }
}
