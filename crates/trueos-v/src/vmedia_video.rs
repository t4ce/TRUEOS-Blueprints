...
fn command(op: u32, a: u64, b: u64, input: &[u8], output: &mut [u8]) -> i32 {
    unsafe { bp_abi::trueos_cabi_vmedia_video_command_v1(op, a, b,
        input.as_ptr(), input.len(), output.as_mut_ptr(), output.len()) }
}
struct Stream { id: u32 }
impl Drop for Stream {
    fn drop(&mut self) { let _ = command(5, self.id as u64, 0, &[], &mut []); }
}
...
pub struct Video { stream: Arc<Stream> }
pub enum VideoPoll { Pending, Frame(VideoFrame), End }
impl Video {
    pub fn open(device: Device, encoded: &[u8], looping: bool) -> Result<Self, i32> {
        ...
        if id <= 0 { return Err(if id == 0 { -3 } else { id }); }
    ...
    pub fn poll(&mut self) -> Result<VideoPoll, i32> {
        ...
        match command(3, self.stream.id as u64, 0, &[], &mut bytes) {
            0 => Ok(VideoPoll::Pending), 2 => Ok(VideoPoll::End),
            1 => Ok(VideoPoll::Frame(VideoFrame { stream: self.stream.clone(),
...
pub struct VideoFrame {
    stream: Arc<Stream>, texture: u64, sequence: u64, width: u32, height: u32,
}
impl VideoFrame {
    pub fn texture_id(&self) -> super::TextureId { super::TextureId(self.texture) }
    pub fn sequence(&self) -> u64 { self.sequence }
    pub fn extent(&self) -> [u32; 2] { [self.width, self.height] }
}
impl Drop for VideoFrame {
    fn drop(&mut self) { let _ = command(4, self.stream.id as u64, self.sequence, &[], &mut []); }
}
