...
type ThermalReadFn = unsafe extern "C" fn(offset: usize, out_ptr: *mut u8, out_cap: usize) -> isize;

unsafe extern "C" {
    fn trueos_vlayer_thermal_snapshot_read(
        offset: usize,
        out_ptr: *mut u8,
        out_cap: usize,
    ) -> isize;
...
pub fn snapshot_bytes() -> Result<Vec<u8>, i32> {
    read_all(trueos_vlayer_thermal_snapshot_read)
}

#[inline]
pub fn snapshot_len() -> Result<usize, i32> {
    read_len(trueos_vlayer_thermal_snapshot_read)
}
...
fn read_all(read_fn: ThermalReadFn) -> Result<Vec<u8>, i32> {
...
fn read_len(read_fn: ThermalReadFn) -> Result<usize, i32> {
