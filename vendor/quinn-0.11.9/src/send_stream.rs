...
impl tokio::io::AsyncWrite for SendStream {
    fn poll_write(
        ...
        return self.poll_write(cx, buf).map_err(Into::into);
        #[cfg(any(target_os = "trueos", target_os = "zkvm"))]
        self.poll_write(cx, buf).map_err(|_| {
            tokio::io::Error::new(tokio::io::ErrorKind::Other, "Quinn stream write error")
        })
    }
    ...
    fn poll_shutdown(self: Pin<&mut Self>, _cx: &mut Context) -> Poll<tokio::io::Result<()>> {
        #[cfg(not(any(target_os = "trueos", target_os = "zkvm")))]
        return Poll::Ready(self.get_mut().finish().map_err(Into::into));
        #[cfg(any(target_os = "trueos", target_os = "zkvm"))]
        Poll::Ready(self.get_mut().finish().map_err(|_| {
            tokio::io::Error::new(tokio::io::ErrorKind::Other, "Quinn stream shutdown error")
        }))
    }
}
