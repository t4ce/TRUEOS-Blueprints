...
lazy_static! {
    ...
    static ref POW43_TABLE: [f32; 8192] = {
        ...
        for (i, pow43) in pow43.iter_mut().enumerate() {
            *pow43 = libm::powf(i as f32, 4.0 / 3.0);
        }
        pow43
    };
}
...
lazy_static! {
    ...
    static ref NORMAL_SCF_TABLE: [f32; 256] = {
        ...
        for (i, table) in table.iter_mut().enumerate() {
            *table = libm::powf(2.0, 0.25 * f32::from(i as i16 - 56 + NORMAL_SCALE_MIN))
        }
        table
    };
}
...
lazy_static! {
    ...
    static ref INTENSITY_SCF_TABLE: [f32; 256] = {
        let mut table = [0f32; 256];
        for (i, table) in table.iter_mut().enumerate() {
            *table = libm::powf(0.5, 0.25 * f32::from(i as i16 + INTENSITY_SCALE_MIN));
        }
        table
    };
}
...
fn decode_noise(lcg: &mut Lcg, sf: f32, dst: &mut [f32]) {
    ...
    let scale = sf / libm::sqrtf(energy);
