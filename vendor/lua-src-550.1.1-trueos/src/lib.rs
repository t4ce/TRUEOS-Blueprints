...
impl Build {
    ...
    pub fn try_build(&self, version: Version) -> Result<Artifacts, Box<dyn Error>> {
        ...
        match target {
            // TRUEOS provides its C ABI at the Blueprint boundary. Keep Lua on
            // its generic ANSI-C path: no signals, dynamic loading, or POSIX
            // process APIs are implied by this target mapping.
            _ if target.contains("trueos") => {}
