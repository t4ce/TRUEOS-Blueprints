...
// Wasi and zkvm don't have a Unix-like user identity model, so this is
// implicitly true until the platform exposes ownership metadata.
#[cfg(any(target_os = "wasi", target_os = "zkvm"))]
mod impl_ {
...
#[cfg(all(not(windows), not(target_os = "wasi"), not(target_os = "zkvm")))]
mod impl_ {
    use std::path::Path;
