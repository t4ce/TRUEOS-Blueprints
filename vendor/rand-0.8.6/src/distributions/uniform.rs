...
/// implementations; for example `rng.r#gen::<u8>() % 170` samples from the range
    ...
    ($ty:ty, $uty:ident, $f_scalar:ident, $u_scalar:ident, $bits_to_discard:expr) => {
        ...
        impl UniformSampler for UniformFloat<$ty> {
            ...
            fn sample<R: Rng + ?Sized>(&self, rng: &mut R) -> Self::X {
                ...
                let value1_2 = (rng.r#gen::<$uty>() >> $bits_to_discard).into_float_with_exponent(0);
                ...
                B2: SampleBorrow<Self::X> + Sized,
            {
                ...
                loop {
                    // Generate a value in the range [1, 2)
                    let value1_2 =
                        (rng.r#gen::<$uty>() >> $bits_to_discard).into_float_with_exponent(0);
